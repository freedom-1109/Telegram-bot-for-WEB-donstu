...
""" запрвшивает погоду с сайта OpenWeatherMap """
def getWeather(city):
    ...
    temperature = f"от {data['main']['temp_min']} до {data['main']['temp_max']}" \
        if {data['main']['temp_min']} != {data['main']['temp_max']} else f"{data['main']['temp_min']}"
...
""" отправляет полученную погоду """
@dp.callback_query_handler(Text(startswith='get') and Text(endswith='Weather'))
async def sentWeather(callback: types.CallbackQuery):
